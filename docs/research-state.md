...
- ただし seed階層 bootstrap は seed集合と再標本化設計に依存する探索的な不確実性指標であり、全 output bit pair の BIC 風指標、入力bit x 出力bit全体の heatmap、より広い入力差分方向はまだ未確認。
