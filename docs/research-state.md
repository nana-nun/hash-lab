...
| `results/2026-05-16-avalanche-mini-sha-bic-all-output-pairs/` | `12, 13, 14, 16` | random input bit mode で全 output bit pair correlation を測定。12 rounds は最大絶対相関 `0.09787985`、Holm補正後 `22` pairs が残り、13/14/16 rounds は補正後 `0` pairs。 |
| `results/2026-05-16-random-like-boundary-comparison/` | comparison | aggregate mean、output bit別SAC、input-output heatmap、BIC、logistic regression distinguisher の random-like 境界を横比較。aggregate mean と random-input BIC では 13 rounds、SAC風・局在・fixed-input BIC では 14 rounds、当時の既存logistic regressionでは 4 rounds と境界がずれていた。 |
...
- seeds `1..20` に増やすと、13 rounds の bit255 は差分が `-0.016050` に弱まるが補正後も baseline `0.5` と矛盾する。seed別 flip rate の bootstrap でも 95% CI は baseline `0.5` を含まない。Holm補正後に残った `225, 228, 231, 254, 255` も seed階層CIではすべて baseline `0.5` を含まず、13 rounds の局所的な残差偏り候補として残る。入力bit位置別測定では、5つの rejected output bits すべてで input bits `224..254` 付近に強い局在偏りが見え、aggregate 測定では正負の偏りが相殺されていた可能性が高い。bit255 を 12/13/14 rounds で横比較すると、入力bit位置別 Holm reject count は `61 -> 32 -> 10` と減り、14 rounds でも aggregate では見えにくい小さな局在偏りが残る。全 input bit x output bit heatmap でも、input bits `224..254` の平均絶対差分は 12/13 rounds で全体平均より大きく、14/16 rounds で全体平均付近まで縮小した。BIC風の fixed input bit 小規模pair correlationでは、rejected output bits の最大絶対相関も 12 rounds `0.729431`、13 rounds `0.566370`、14 rounds `0.067618` と弱まった。random input bit mode の全 output bit pair correlation では、12 rounds のみ Holm補正後に `22` pairs が残り、13/14/16 rounds は補正後 `0` pairs だった。
- 複数指標の横比較では、random-like に見え始めるroundは指標ごとにずれる。aggregate mean と random-input BIC では 13 rounds、output-bit SAC風指標・input-output局在・fixed-input small BIC では 14 rounds が境界に見える。logistic regression distinguisher は 4/8/9..15/16 rounds で baseline付近に留まるが、32 rounds では小さな正方向deltaが出たため追加確認が必要である。
...
- ただし seed階層 bootstrap は seed集合と再標本化設計に依存する探索的な不確実性指標であり、fixed input bit 条件での全 output bit pair correlation、heatmap 上位cellの追加samples確認、より広い入力差分方向はまだ未確認。
...
| `results/2026-05-16-distinguish-9-15-32/` | `9, 10, 11, 12, 13, 14, 15, 32` | seeds `1..5`、samples `2000`、epochs `8`。9..15 rounds は seed階層bootstrap CI が `0` を含む。32 rounds は平均delta `0.0103`、95% CI `[0.0003, 0.0203]` で小さな正方向差分が出た。 |
...
- 9..15 rounds でも logistic regression の test accuracy は baseline付近に留まった。9/10/11/12 rounds は avalanche mean やSAC風指標で偏りが残るが、digest-vs-random-bit列の線形distinguisherでは汎化する差として見えなかった。
- 32 rounds は平均 `test_accuracy_minus_baseline = 0.0103` で seed階層bootstrap CI が `0` を含まなかった。ただし seed5個・単一split設計の小さな差分なので、追加seedや複数splitで再確認するまでは探索的な観察として扱う。再確認は #104 で扱う。
...
- 32 rounds の logistic regression の小さな正方向deltaは、追加seedや複数splitでも再現するか: #104
