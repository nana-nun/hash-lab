...
- 9から12 rounds で mean flip ratio は急速に `0.5` へ近づく。12 rounds はCI込みでも `0.5` より低く、bit位置別にも最大差分 `0.046000` が残る。13 rounds は seed階層CIでは baseline `0.5` と矛盾しにくく、bit位置別 min/max も 12 rounds より大きく狭まる。
