...
- seeds `1..20` に増やすと、13 rounds の bit255 は差分が `-0.016050` に弱まるが補正後も baseline `0.5` と矛盾する。seed別 flip rate の bootstrap でも 95% CI は baseline `0.5` を含まない。Holm補正後に残った `225, 228, 231, 254, 255` も seed階層CIではすべて baseline `0.5` を含まず、13 rounds の局所的な残差偏り候補として残る。bit255 の入力bit位置別測定では、偏りは input bits `224..254` 付近に強く局在し、aggregate 測定では正負の偏りが相殺されていた可能性が高い。
...
- ただし seed階層 bootstrap は seed集合と再標本化設計に依存する探索的な不確実性指標であり、BIC 風指標や入力bit位置ごとの条件付き偏りはまだ未確認。
