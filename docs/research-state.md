...
- 9から12 rounds で mean flip ratio は急速に `0.5` へ近づく。12 rounds はCI込みでも `0.5` より低く、13 rounds は seed階層CIでは baseline `0.5` と矛盾しにくい。
- ただし seed階層 bootstrap も seed数5個の探索的な指標であり、bit位置ごとの小さな差分の信頼区間、BIC 風指標はまだ未確認。
...
- 出力bit位置ごとの小さな差分は sampling noise と矛盾するか。
