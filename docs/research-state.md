...
| `results/2026-05-16-random-like-boundary-comparison/` | comparison | aggregate mean、output bit別SAC、input-output heatmap、BIC、logistic regression distinguisher の random-like 境界を横比較。aggregate mean と random-input BIC では 13 rounds、SAC風・局在・fixed-input BIC では 14 rounds、既存logistic regressionでは 4 rounds と境界がずれる。 |
...
- 複数指標の横比較では、random-like に見え始めるroundは指標ごとにずれる。aggregate mean と random-input BIC では 13 rounds、output-bit SAC風指標・input-output局在・fixed-input small BIC では 14 rounds が境界に見える。既存の logistic regression distinguisher は 4/8/16 rounds が baseline付近だが、9-15 rounds と32 rounds は未測定である。
