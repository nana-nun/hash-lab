...
- In the Codex environment, `.git` may contain sandbox-related Deny ACLs that prevent normal-permission Git from creating `.git/index.lock` or `.git/ORIG_HEAD.lock`. Run Git commands that write to `.git` with the approved/escalated execution path from the start, including `git fetch`, `git switch`, `git merge`, `git add`, `git commit`, and `git push`. Read-only commands such as `git status`, `git diff`, `git log`, and `git show` can use normal permissions.
- If a git command still fails with `.git/*.lock` `Permission denied`, retry the same git operation with the approved/escalated execution path. Do not delete lock files unless a human explicitly asks and the resolved path has been checked.
