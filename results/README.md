...
| `2026-05-12-avalanche-mini-sha-13-rejected-bits-input-localization/` | avalanche input bit positions | `seed_input_bit_metrics.csv`, `input_bit_ci_metrics.csv`, `summary.csv`, `compute_rejected_bits_input_localization.py`, `config.json`, `notes.md` | 13 rounds の rejected output bits `225, 228, 231, 254, 255` を入力bit位置別に比較。5bitすべてで input bits `224..254` 付近に強い局在偏りが見える。 |
| `2026-05-12-avalanche-mini-sha-12-14-bit255-input-bits/` | avalanche input bit positions | `seed_input_bit_metrics.csv`, `input_bit_ci_metrics.csv`, `summary.csv`, `compute_round_input_bit_bias.py`, `config.json`, `notes.md` | 12/13/14 rounds の bit255 を入力bit位置別に比較。Holm reject count は `61 -> 32 -> 10` と減り、14 rounds でも input bits `226..254` 付近に小さな局在偏りが残る。 |
| `2026-05-16-avalanche-mini-sha-input-output-heatmap/` | avalanche input-output heatmap | `flip_rate_matrix.csv`, `baseline_delta_matrix.csv`, `summary.csv`, `round_*_baseline_delta_heatmap.png`, `compute_input_output_heatmap.py`, `config.json`, `notes.md` | rounds 12/13/14/16 の全 input bit x output bit flip rate を探索的に測定。12/13 rounds では後半 input bits の局在偏りが強く、14/16 rounds で大きく縮小。 |
| `2026-05-14-avalanche-mini-sha-bic-rejected-bits/` | avalanche bit pair correlation | `avalanche_vectors.csv`, `pair_correlation.csv`, `summary.csv`, `compute_bic_pair_correlation.py`, `config.json`, `notes.md` | `225, 228, 231, 254, 255` の output bit pair correlation を fixed input bits 小集合で測定。最大絶対相関は 12 rounds `0.729431`、13 rounds `0.566370`、14 rounds `0.067618`。 |
| `2026-05-16-avalanche-mini-sha-bic-all-output-pairs/` | avalanche bit pair correlation | `pair_correlation.csv`, `top_pairs.csv`, `summary.csv`, `round_*_pair_correlation_heatmap.png`, `compute_bic_all_output_pairs.py`, `config.json`, `notes.md` | rounds 12/13/14/16 の全 output bit pair `32640` pairsを random input bit mode で測定。12 rounds は Holm補正後 `22` pairs、13/14/16 rounds は `0` pairs。 |
| `2026-05-16-random-like-boundary-comparison/` | comparison | `round_metric_comparison.csv`, `boundary_summary.csv`, `compare_random_like_boundaries.py`, `config.json`, `notes.md` | aggregate mean、output bit別SAC、input-output heatmap、BIC、logistic regression distinguisher の random-like 境界を横比較。指標ごとに 13 rounds、14 rounds、4 rounds と境界がずれる。 |
